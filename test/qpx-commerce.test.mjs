...
test('buildAddToCartEvent: Mengen-Erhoehung 1->3 zaehlt nur das Delta (2)', () => {
  const ev = buildAddToCartEvent({
    currentLine: {
      quantity: 3,
      merchandise: {
...
test('qpxTrack: puffert ueber window.qpx.q, wenn qpx noch nicht geladen', () => {
...
// ── Verdrahtung: Standard-PDP traegt die 3 data-section-Anker ───────────────
test('products.$handle.jsx: StandardProduct traegt pdp-section-Anker', () => {
...
test('products.$handle.jsx: pdp-Sektionen sind DISJUNKT (nicht verschachtelt)', () => {
  const src = readFileSync(join(APP, 'routes', 'products.$handle.jsx'), 'utf8');
  // product-main traegt KEINE data-section (sonst umschliesst es description+buybox):
  assert.match(
    src,
