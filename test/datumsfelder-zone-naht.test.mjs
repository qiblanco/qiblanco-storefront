// Die NAHT zwischen isoMitZone() und den Flaechen, die ein Datum auszeichnen
...
//
// WAS DIESE DATEI PRUEFT UND WAS NICHT. test/datum-iso-mit-zone.test.mjs prueft
// die FUNKTION, und sie ist gruen, seit es sie gibt. Gruen war damals auch jede
// Seite — die Funktion wurde nur nirgends aufgerufen. Genau diese Naht reisst
// in diesem Haus am haeufigsten: zwei gruene Seiten, eine kaputte Verbindung.
...
//
// DIE GRENZE DIESER DATEI, damit sie niemand fuer mehr haelt, als sie ist: sie
// ist eine Rueckfall-Sperre fuer die Aenderung von s02, KEIN Zensus ueber alle
// Datumsfelder des Repos. Die stehende Wache ueber die ganze Klasse entsteht in
// Segment s03 im Flaechen-Zensus des seo-manager.
...
// die deshalb nicht ueber `standFuer()` geheilt werden koennen.
const ROUTEN = [
...
test('standFuer liefert den Zeitstempel — und verschiebt den Kalendertag nicht', () => {
  ...
  // Tag verschiebt, waere schlimmer als gar keine.
...
test('KEINE ERFUNDENE GENAUIGKEIT: fremde Publikationsdaten bleiben unberuehrt', () => {
  ...
  // (die Funktion laesst sie durch), sondern weil der Aufruf die Absicht
  // faelschlich als "wir haetten hier gern eine Uhrzeit" liest.
