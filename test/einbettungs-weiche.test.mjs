...
test('W5 entry.server.jsx antwortet mit der gemeinsamen Weiche VOR dem Rendern', () => {
  const src = readFileSync(join(WURZEL, 'app/entry.server.jsx'), 'utf8');
  const w = src.indexOf('einbettungsWeicheGemeinsam(request)');
  const r = src.indexOf('await renderToReadableStream(');
  assert.ok(w > 0, 'Aufruf fehlt');
  assert.ok(w < r, 'Weiche steht hinter dem Rendern');
  assert.match(src.slice(w, w + 120), /if \(weiterSeite\) return weiterSeite;/);
});
