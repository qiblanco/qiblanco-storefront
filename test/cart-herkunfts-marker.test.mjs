...
// dafuer per module.registerHooks aufgeloest (Bordmittel seit Node 22.15).
...
registerHooks({
  resolve(spezifizierer, kontext, naechster) {
    if (spezifizierer.startsWith('~/')) {
      ...
      // ACHTUNG: nicht auf "irgendeine Endung" pruefen — '~/lib/
  ...
  load(url, kontext, naechster) {
    ...
    // Komponenten geben null zurueck). Node kennt die Endung nicht und
    // braucht das Format ausdruecklich genannt.
    if (url.endsWith('.jsx')) {
      return {
        format: 'module',
        shortCircuit: true,
...
function pruefeHerkunftImmer(attrs, marker) {
  ...
  for (const key of HERKUNFT) {
    assert.ok(
      m.has(key),
      `${marker}: Herkunfts-Marker "${key}" fehlt — der Frueh-Ausstieg ist zurueck`,
    );
...
test('ARM-B2 MIT Consent: der Kassen-Knopf traegt zusaetzlich die Klick-ID', async () => {
  const {spur, cart} = cartAttrappe();
  await attributionAction({
    request: anfrage({cookie: `${COOKIE_CONSENT_JA}; ${COOKIES_PERSONENBEZOGEN}`}),
    context: {cart, env: {}},
...
test('ARM-C2 MIT Consent: der Direktlink traegt zusaetzlich die Klick-ID', async () => {
  const {spur, cart} = cartAttrappe();
  await linesLoader({
    request: anfrage({
      url: 'https://qiblanco.com/cart/41007289663544:1?fbclid=IwZXh0bgNhZW0BMABhTESTKLICK',
...
test('ARM-D1 ua_class: zeichengleich mit dem Bestands-Klassifikator (hyros-eigenbau basis.py ua_klasse)', () => {
  ...
  // hyros-eigenbau/receiver/src/basis.py::ua_klasse fuer dieselben Eingaben
  // (Paritaet 18/18 gemessen 2026-09-07). Ein zweiter, abweichender
  // Klassifikator waere in der spaeteren Auswertung nicht mehr zu trennen.
  const faelle = [
  ...
  for (const [ua, erwartet] of faelle) {
    assert.equal(
      ...
      erwartet,
      `ARM-D1: Klassifikator weicht vom Bestand ab fuer "${String(ua).slice(0, 50)}"`,
    );
  }
});

test('ARM-D2 ua_class: leerer User-Agent ergibt NIE "browser"', () => {
  // Der Bestand begruendet das ausdruecklich: ein Default auf 'browser' waere
...
// lesen (was gegen genau diesen Defekt blind waere — der Code STAND ja da),
// laeuft er hier in node:vm gegen eine Mini-DOM-Attrappe. Der Preview-Schalter
// data-qiblanco-tracking-preview oeffnet trackingAllowed() ohne Cookiebot.
...
function trackerLaufen({search, cookiesVorher, gepuffertesFbclid = ''}) {
  ...
  // hatte). Eine Pruefung, die nicht rot werden kann, belegt nichts.
  ...
  try {
    ...
    // kennt. Fuer diesen Arm zaehlt allein, was bis dahin im Cookie-Jar steht.
  }
  return jar.get('_fbc') || '';
}
...
test('ARM-E1 DER FIX: neuer Ad-Klick frischt ein veraltetes _fbc auf', () => {
  ...
  assert.equal(
    fbclidAus(fbc),
    FBCLID_NEU,
    'ARM-E1: das _fbc traegt weiter die ALTE fbclid — der Frueh-Ausstieg `if (readCookie("_fbc")) return;` ist zurueck, und die Klick-Identitaet friert 90 Tage ein',
  );
});
...
test('ARM-E2 GEGENRICHTUNG: gleiche fbclid wird NICHT neu gestempelt', () => {
  ...
  // Lookback-Fenster liesse sich nie schliessen.
...
test('ARM-E3 GEGENRICHTUNG: ein GEPUFFERTER fbclid darf ein vorhandenes _fbc NIE ueberschreiben', () => {
  ...
  // aeltere zu ersetzen. Der Puffer ist hier ECHT bestueckt und traegt eine
  // ANDERE fbclid als das Cookie; ohne beides waere der Arm gruen by
  // construction und wuerde nichts belegen.
  const vorher = `fb.1.1000000000.${FBCLID_ALT}`;
  const fbc = trackerLaufen({
    search: '',
    cookiesVorher: {_fbc: vorher},
