// Regressionstest zum Frühausstieg in buildAttributionCartAttributes
...
// Ausführen: node --test test/checkout-tracking-signallos.test.mjs
//
...
//   VOR dem Marker: ein signal-loser Besucher verließ die Funktion, bevor
//   irgendetwas geschrieben wurde. Damit war "Order lief nicht über die
...
// diesen Frühausstieg bereits geschlossen: Marker UNBEDINGT, aber der Marker
// zählt NICHT als Signal (sonst würden landing_page/referrer plötzlich für
// jeden organischen Besucher geschrieben). Diese Tests halten BEIDE Hälften.
...
test('DER FIX: signal-loser Besucher trägt den Marker (vorher: gar nichts)', () => {
  ...
  assert.ok(
    marker,
    'signal-loser Besucher bekam KEIN attribution_source — der Frühausstieg ist zurück',
  );
...
test('DIE ANDERE HAELFTE: der Marker zählt NICHT als Signal', () => {
  // Ein organischer Besucher hat den Attributions-Cookie (href/referrer), aber
  // KEIN Ad-Signal. landing_page/referrer dürfen deshalb NICHT geschrieben
  // werden — sonst hätte der Fix die Datenmenge still ausgeweitet.
  const attrs = buildAttributionCartAttributes({
    searchParams: new URLSearchParams(),
  ...
  assert.ok(
    ...
    'landing_page wurde ohne Ad-Signal geschrieben — der Marker gilt fälschlich als Signal',
  );
  assert.ok(
    !keys.has('referrer'),
    'referrer wurde ohne Ad-Signal geschrieben — der Marker gilt fälschlich als Signal',
  );
  assert.ok(
...
test('KEINE REGRESSION: _qpx_anon aus dem Cookie zählt weiter als Signal', () => {
  // Der Stitch-Schlüssel selbst ist ein Signal — ein Besucher mit _qpx_anon
