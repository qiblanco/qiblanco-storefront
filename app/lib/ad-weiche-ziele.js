 ...
 * utm_medium=paid, url_host=qiblanco.com, 14 Tage): ueber 99 Prozent der
 ...
 * (adIdAusQuery: utm_content, sonst h_ad_id; 98,7 Prozent Deckung ueber
 ...
 * (es fehlt keine — fuer 10 der 11 aktiven DE-Anzeigen existiert eine
 ...
 * zuteilung.json fuehrt bereits eine Ad->LP-Tabelle (Feld `herkunft`,
 * 18 Eintraege). Sie taugt hier NICHT als Traeger, aus zwei gemessenen
 * Gruenden: (1) sie wird vom taeglichen lp-rotation-Tick frisch berechnet
 * (src/zuteilung.py baue_inhalt) und ist KEIN Hand-Feld — ein Eintrag von
 * Hand waere am naechsten Morgen um 05:35 weg; (2) ihre Werte stammen aus
 ...
 * ist fuer die Attribution gedacht — die ARM-TRENNUNG fuer die
 * Vorher-Nachher-Messung haengt NICHT an ihm, sondern am Landepfad:
 * events.db speichert in url_path keinen Query-String (0 Zeilen mit '?'),
 * ein lp_m-Wert waere dort also gar nicht lesbar. */
 ...
 * bestandenen Beleg gehoert NICHT in diese Karte — sonst wuerde die Weiche
 * bezahlten Verkehr auf eine Flaeche lenken, die das eigene Gate nicht
 * besteht, und der Fehler von LP A waere nur verschoben.
 */
export const MM_ZIELE = Object.freeze({
  // --- Welle B3 "Alltagsdemo", headline "Haelt das meinen Alltag aus?"
  ...
  // --- Welle C1 "Aufmachen", headline "Jetzt weisst du, was du traegst"
  //     Versprechen: "Ist das nicht ein Scam? Faire Frage — also machen wir
  //     ihn auf": Gitterchip, Chirurgenstahl, Bayern, praeklinisch in vitro.
 ...
 *    Beauty-Gate. Eine Zuordnung waere hier eine Verschlechterung.
 *  - 120251220869070704 "DE | TOF-C | Unternehmer-Hook2" — 741 Erst-Landungen/14d
 *    und damit der groesste Posten, den diese Karte haette tragen koennen.
 ...
 *    exakt gleichauf mit LP A. Die Anzeige dorthin zu leiten waere die
 *    Wiederholung des Defekts, den dieser Bau behebt: bezahlter Verkehr auf
 *    eine Flaeche, die das eigene Gate nicht besteht. Kommt zell-schutz ueber
 *    80, gehoert diese Zeile zurueck — die Befundliste dafuer steht im RESULT.
 *    LEHRE: ein design-review.json traegt ein DATUM, keinen Zustand.
 ...
 *    Marke, nicht eine Frage. LP A ist fuer sie das richtige Ziel.
 ...
 * drueben in HAND_FELDER steht — sonst loescht der taegliche Tick es um 05:35.
 ...
 * bereits seine blosse ERWAEHNUNG in einem Kommentar machte diese Datei fuer
 ...
 * Schutz des Feldes waere still unbewacht. Deshalb liegt der Feldzugriff
...
/**
 * Reine Entscheidungsfunktion: welcher LP-Pfad gilt fuer diese Anzeige?
 ...
 * Weiche feuert auf JEDER Dokument-Route. Ohne ihn wuerde eine zugeordnete
 ...
 * genau diese Pruefung fuer jede Route, die ihrerseits weiterleitet.
 * Er ersetzt zugleich einen Eintrag in AUSSCHLUSS_SEGMENTE: ein dortiger
 * Eintrag wuerde die Weiche auf diesen Seiten fuer ALLE Anzeigen abschalten,
 * auch fuer die nicht zugeordneten — diese Pruefung wirkt nur dort, wo sie
 * muss.
 *
...
export function mmZielPfad(aktuellerPfad, adId, searchParams, aktiv, zufall = Math.random) {
  ...
  // Quell-URL fuer JEDES Messwerkzeug mehrdeutig. Ohne Pin waere die
  // Abnahme-Probe auf einen Zufallsarm angewiesen, und der taegliche
  // Design-Watch koennte der Weiche folgen und den Beleg der falschen Seite
