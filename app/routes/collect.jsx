...
/**
 * Header fuer den Weiterreich-Fetch an den Receiver.
 *
 ...
 * an. Gemessene Folge: 0 von 11.037 DACH-Events mit device_fp/ua_hash, waehrend
 ...
 *
 * Zwei Header, zwei verschiedene Empfaenger im Receiver:
 *   User-Agent      -> fingerprint.ua_hash()/device_fp()/device_class()
 *   X-Forwarded-For -> basis.client_ip() nimmt den LINKESTEN Eintrag. Caddy
 *                      haengt seinen Peer rechts an, links bleibt unsere
 *                      Client-IP. Ohne diesen Header stand dort die Egress-IP
 *                      des Oxygen-Edge — der ip_net_hash war also nicht bloss
 *                      duenn, sondern FALSCH (gemessen: 190 verschiedene
 *                      Besucher hinter EINEM Hash).
 *
 * IP-Quelle ist `CF-Connecting-IP` (vom Edge gesetzt, client-seitig NICHT
 * faelschbar) mit dem repo-ueblichen XFF-Rueckfall (vgl. pages.support.jsx).
 * Leere Werte werden WEGGELASSEN statt als leerer Header gesendet: der
 * Receiver soll "nicht gemessen" von "gemessen und leer" unterscheiden koennen.
 *
