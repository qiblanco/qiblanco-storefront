 ...
 * 27.09.2026).
 *
 * Diese Datei enthaelt DREI Bausteine und EIN Overlay:
 *   <EuLabelProvider>            -- haelt genau EINEN <dialog> je Seite
 *   <EuGewaehrleistungsHinweis>  -- Produktseite: NUR der Text-Link
 *   <EuGewaehrleistungsLink>     -- Footer: NUR der Text-Link
 *
 * Beide Ausloeser oeffnen dasselbe Overlay. Die amtliche Grafik erscheint
 * AUSSCHLIESSLICH dort -- nirgends offen im Seitenfluss.
 *
 * ====================================================================
 ...
 * Produktseite OFFEN und benutzte das Overlay nur als Lupe. Begruendet war
 ...
 * erlaubt und fuer die Mitteilung (Anhang I) schweigt.
 *
 * Dieser Schluss war zu eng. Die Kommission selbst beschreibt fuer die
 ...
 *    mouse roll-over."
 *
 * Derselbe Satz steht dort noch zweimal -- fuer den Website-Header und fuer
 ...
 * verschachtelte Format weiterhin nur fuer die Kennzeichnung. Wer diesen Bau
 * uebernimmt, folgt dem Verstaendnis der Kommissionsdienststelle -- ein
 * Restrisiko abweichender nationaler Auslegung bleibt und gehoert vor dem
 * Livegang auf einen juristischen Tisch, nicht auf diesen.
 *
 * ====================================================================
 * WAS DAS FUER DEN QR-CODE BEDEUTET
 * ====================================================================
 ...
 * 2 x 2 cm (Abschnitt 3.1.2 -- dort fuer das GARAN-Label geschrieben, hier
 * bewusst auf die Mitteilung uebertragen, weil es fuer deren QR-Code keine
 * eigene Zahl gibt).
 *
 ...
 * beschaedigt, dass sich der QR-Code darin nicht einmal lokalisieren laesst
 * (siehe QR_DEFEKT). Werden sie neu hochgeladen, ist dieser Anteil
 * nachzumessen -- faellt er unter 18,24 %, traegt die Mindestbreite unten
 * nicht mehr. Die CSS setzt darum eine MINDESTBREITE fuer die Grafik im
 ...
 * ====================================================================
 * `showModal()` bringt Fokusfalle, Esc-Schliessen und -- der eigentliche
 * Punkt -- die FOKUS-RUECKGABE an das ausloesende Element von sich aus mit.
 * Ein handgebauter Nachbau muesste all das selbst halten und verliert die
 * Rueckgabe erfahrungsgemaess als Erstes. Der Auftrag verlangt sie
 * ausdruecklich.
 */
...
/**
 * Haelt genau EINEN <dialog> fuer die ganze Seite. Gehoert in root.jsx um
...
export function EuLabelProvider({children}) {
  ...
  const oeffnen = useCallback(() => {
    ...
    // oder zweiter Ausloeser). Ohne den Schutz reisst das die Seite ab.
  ...
  const wert = useMemo(() => ({oeffnen, schliessen, label}), [oeffnen, schliessen, label]);

  return (
    <EuLabelKontext.Provider value={wert}>
      {children}
      <EuLabelDialog ref={dialogRef} label={label} onSchliessen={schliessen} />
...
// (package.json). Die React-19-Schreibweise "ref als normales Prop" waere
// hier still `undefined` -- showModal() liefe nie, das Overlay bliebe tot,
// und der Fehler zeigte sich erst im Browser.
const EuLabelDialog = forwardRef(function EuLabelDialog({label, onSchliessen}, ref) {
  // Klick auf den dunklen Rand schliesst. Der <dialog> selbst IST der
  // zentrierte Kasten (der Rand ist ::backdrop), deshalb laesst sich der Rand
  // nicht direkt beklicken -- die Trefferpruefung laeuft ueber die Geometrie
  // des Kastens. Ein naives onClick am <dialog> wuerde auch bei einem Klick
  // auf das Bild schliessen.
    ...
    >
      <div className="eu-gwl-dialog__kasten">
        ...
        {/*
          ...
          Schliessen-Knopf stehen, wenn bei schmalem Viewport seitlich
          geschoben wird.
        */}
