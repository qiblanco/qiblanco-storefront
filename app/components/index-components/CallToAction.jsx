 ...
 */
/*
 * DAS `sizes`-ATTRIBUT BESCHREIBT DIE GEMESSENE FLÄCHE, NICHT EINE GEWÜNSCHTE
 ...
 * längst die Sprossen 659w, 1318w und 1977w an — der Browser wählte die
 * 659er nur deshalb, weil `sizes` ihm eine 659-px-Fläche behauptete. Gemessen
 * am Kundenrand waren es 968 px. Gate 12 meldete daraufhin auf drei
 * Detailseiten "659px Quelle auf 968px*1x Fläche". Es fehlte also kein Asset
 * und keine Leiter — es fehlte eine wahre Angabe.
 *
 * DIE FLÄCHE, HERGELEITET UND DANN NACHGEMESSEN:
 ...
 *         und dann füllt JEDES Kind die volle Inhaltsbreite.
 *
 * WO DIE ZEILE UMBRICHT — UND WARUM NICHT AM TEXT: `flex: 1 0 50%` heißt
 * flex-shrink 0, das Bild kann also nicht kleiner werden als seine
 * automatische Mindestgröße, und die ist das `width={500}` eine Zeile
 * darüber. Unterschreitet die halbe Inhaltsbreite diese 500 px, passt die
 ...
 * (2026-09-21, drei Bausteine auf zwei Seiten). Das ist wichtig für die
 * Haltbarkeit dieser Zahl — 1032 hängt an einer BAULICHEN Größe (dem
 * width-Attribut), nicht an einer redaktionellen. Ein Textwechsel veraltet sie
 * nicht; eine Änderung an `width` oder am Innenabstand schon.
 *
 * GEGENPROBE, live gemessen (dpr 1, getBoundingClientRect, alle drei
 * Detailseiten): 360->328 | 414->382 | 600->568 | 1000->968 | 1030->998 ||
 * 1032->500,0 | 1050->509 | 1100->534 | 1200->584 | 1279->623,5 | 1300->634 |
 * 1350->659 | 1400->659. Der Sprung liegt auf den Pixel dort, wo die Rechnung
 * ihn vorhersagt, und der erste zweispaltige Wert ist exakt die 500 aus dem
 * width-Attribut. Der neue Ausdruck trifft JEDEN dieser Werte; das alte
 * "92vw" traf keinen einzigen (360: 91,1 % | 600: 94,7 % | 1000: 96,8 %).
 *
 * WARUM `anzeigeBreite` BEI 659 BLEIBT, OBWOHL DIE FLÄCHE GRÖSSER SEIN KANN:
 ...
 * ein 360-px-Handy bei dpr 2 die 659er. Auf 992 gesetzt, wäre die unterste
 * Sprosse 992 px — das Handy bekäme unnötig das Anderthalbfache. WER DIE
