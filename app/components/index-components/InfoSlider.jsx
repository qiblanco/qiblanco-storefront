...
export function InfoSlider({dataSection}){
    ...
    const measureSlideStep = useCallback(() => {
        ...
        const firstCard = track?.querySelector(".InfoSliderCard");
        const cardWidth = firstCard?.getBoundingClientRect().width || 400;
        const trackStyles = track ? window.getComputedStyle(track) : null;
        const gap = parseFloat(trackStyles?.columnGap || trackStyles?.gap || "20") || 20;

        return cardWidth + gap;
    }, []);

    useEffect(() => {
        const updateStep = () => setSlideStep(measureSlideStep());
    ...
    const handlePlacementBehaviour = (direction) => {
        setActiveSlide((current) => {
            if (direction === "next") {
                return current < maxSlideIndex ? current + 1 : 0;
            }
...
function InfoSliderCard({title, label, description, background, shouldSuppressClick}){
