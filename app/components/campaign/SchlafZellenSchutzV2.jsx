 ...
 *
 * MESS-DISZIPLIN (qpx zaehlt JEDEN Klick im naechsten [data-section]-Vorfahr,
 * public/qiblanco-qpx.js Z. 376-380): Sektionen, die in die Zwischen-Conversion
 * eingehen (hero, kaufblock, haltung, final), enthalten AUSSCHLIESSLICH
 ...
 * deskriptiv, Geld-zurueck an den ZEITRAUM gebunden (nie ans Spueren).
 ...
 * die R1/R2/R3-Kette haengt pfad-agnostisch im root-Layout (D-006).
 */
...
/* ───────── 1 HERO ─────────────────────────────────────────────────────────
   ...
   Dreizeiler mit „Mehr Energie." ZUERST — s02-Mismatch 1: der haeufigste
...
/* ───────── 2 EBENEN (JTBD-Selector, R5: inline statt ausleiten) ───────────
   ...
   das Drei-Themen-Band, aber der Klick fuehrt NICHT mehr auf eine Themen-LP,
   sondern in die Wirkkette derselben Seite (Anker). Micro-Commitment
   (Cialdini Konsistenz) + trennscharfes Feature fuer den s03-Recluster. */
const EBENEN = [
...
/* ───────── 4 BEWEIS (R3: aus drei Widgets wird EIN kuratierter Block) ─────
   Enthaelt in dieser Reihenfolge: gemessene Zellstudien-Zahlen (Dwell-Sieger
  ...
  return (
    <section data-section="lp-v2-beweis">
      ...
      {/* Autoritaet MIT NAMEN (KH-4). Bewusst OHNE Portrait und OHNE erfundenes
          ...
          Zitat ohne Transkript-Beleg waere ein Claim-Verstoss (F-003/F-007).
...
/* ───────── 5 KAUFBLOCK (DER Hebel-1-Zug) ─────────────────────────────────
   ...
   QiBracelet ist EIN Produkt (drei Groessen) = genau EINE Karte.
   Der Garantie-Aufklapper haengt in einem EIGENEN data-section-Anker, damit
...
/* ───────── 6 MIKROSKOP (Scrub auf 320vh gekappt, R4) ──────────────────────
   ...
   Design-Meister-Lernkarte jubi2x („320vh erhaelt den Scrub-Effekt und passt
...
/* ───────── 8 HALTUNG (Marken-Abbinder, jetzt MIT Klickziel) ───────────────
   s02-Mismatch 2: Das Status-Versprechen der staerksten Ad wird auf LP A fuer
